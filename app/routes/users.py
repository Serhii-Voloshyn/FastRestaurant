...
@router.post("/token", response_model=schemas.Token, status_code=status.HTTP_200_OK, name="login")
...
@router.get("/me/", response_model=schemas.User, status_code=status.HTTP_200_OK, name="user_me")
...
async def create_user(payload: schemas.User, response: Response, db: Session = Depends(get_db)):
...
async def read_users_all(db: Session = Depends(get_db)):
    ...
    return {'message': 'Successfully retreived', 'results': len(users), 'users': users}
